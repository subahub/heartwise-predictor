...
import { Activity, Download, Trash2, FileText, FileSpreadsheet, FileDown, FileType } from 'lucide-react';
...
export default function History() {
  ...
  const exportPDF = async () => {
    try {
      ...
      doc.text('Cardiovascular Health History Report', 15, 24);
      doc.text(`Generated: ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`, 15, 30);

      doc.setTextColor(40, 40, 40);
      let y = 45;
      doc.setFontSize(13); doc.setFont('helvetica', 'bold');
      doc.text('Patient Summary', 15, y); y += 8;
      doc.setFontSize(10); doc.setFont('helvetica', 'normal');
      doc.text(`Patient: ${user?.name || user?.email || 'N/A'}`, 15, y);
      doc.text(`Total Assessments: ${history.length}`, pageWidth / 2, y); y += 6;

      if (history.length > 0) {
        const latest = history[history.length - 1];
        doc.text(`Latest Score: ${latest.score}/100 (${latest.level.charAt(0).toUpperCase() + latest.level.slice(1)} Risk)`, 15, y);
        doc.text(`Latest BP: ${latest.ap_hi}/${latest.ap_lo} mmHg`, pageWidth / 2, y); y += 6;
        const bmi = (latest.weight / ((latest.height / 100) ** 2)).toFixed(1);
        doc.text(`BMI: ${bmi}`, 15, y); doc.text(`Age: ${latest.age}`, pageWidth / 2, y);
      }

      y += 12;
      doc.setFontSize(13); doc.setFont('helvetica', 'bold');
      doc.text('Assessment History', 15, y); y += 4;
      ...
      doc.save('cardioguard_history_report.pdf');
  ...
  const exportDOCX = async () => {
    try {
      ...
      const headerRow = new TableRow({
        children: ['Date', 'Score', 'Risk Level', 'Age', 'BP', 'Cholesterol', 'BMI'].map(h =>
          new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: h, bold: true, size: 20, color: 'FFFFFF' })] })], borders: cellBorders, shading: { fill: '14535D' } })
        ),
      });

      const dataRows = history.slice().reverse().map(h =>
        new TableRow({
          children: [
            new Date(h.date).toLocaleDateString(),
            String(h.score),
            ...
            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: val, size: 20 })] })], borders: cellBorders })
          ),
        })
      );
      ...
      const doc = new Document({
        sections: [{
          children: [
            ...
            new Paragraph({ children: [new TextRun({ text: 'Health History Report', size: 24, color: '666666' })], alignment: AlignmentType.CENTER, spacing: { after: 50 } }),
            new Paragraph({ children: [new TextRun({ text: `Generated: ${new Date().toLocaleString()}`, size: 20, color: '999999' })], alignment: AlignmentType.CENTER, spacing: { after: 300 } }),

            new Paragraph({ text: 'Patient Summary', heading: HeadingLevel.HEADING_2, spacing: { before: 200, after: 100 } }),
            new Paragraph({ children: [
              new TextRun({ text: `Patient: ${user?.name || user?.email || 'N/A'}`, size: 22 }),
              new TextRun({ text: `   |   Total Assessments: ${history.length}`, size: 22 }),
            ]}),

            new Paragraph({ text: 'Assessment History', heading: HeadingLevel.HEADING_2, spacing: { before: 300, after: 100 } }),
      ...
      saveAs(blob, 'cardioguard_history.docx');
      ...
      <Dialog open={showExportDialog} onOpenChange={setShowExportDialog}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-2 py-2">
            ...
            <Button variant="outline" className="w-full justify-start gap-3" onClick={exportDOCX}>
              <FileType className="h-5 w-5 text-blue-600" /> Word (.docx)
